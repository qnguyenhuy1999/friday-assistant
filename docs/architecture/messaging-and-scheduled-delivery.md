...
The plan freezes the route fingerprint, route-specific maximum body length, and execution lineage, or a stable suppressed reason when the route is missing or disabled. Policy and route changes therefore affect future fires only; historical fires receive no migration backfill and are never retroactively authorized. A pre-bound historical plan whose length authority is unavailable fails closed.

Step 7 materializes delivery intent during the existing bounded worker maintenance tick. For a `READY` plan it resolves the latest Run in the frozen execution lineage, and only a successful effective Run with its canonical `AGENT_FINISHED.summary` may produce one `OutboundDelivery`. The summary crosses a deterministic Friday-owned content gate (line-ending normalization, non-empty, control-character, and size checks); `details` and all route, claim, failure, and event metadata are never outbound content. Active retry lineages remain eligible for later ticks, while terminal failed executions create no message.

The delivery uses only the plan's frozen route id, fingerprint, and body bound. There is exactly one durable intent per fire, fenced by `OutboundDelivery.source_schedule_fire_id`; materialization does no model or network I/O. A permanently rejected canonical result records only the effective run id, so it cannot starve later candidates; a later retry has a different run id and becomes eligible normally. Dispatch remains a separate existing component, and it validates route drift against current authority before transport. Historical fires with no Step 6 plan never gain delivery authority retroactively.

## Phase 19 closure: outbound messaging

Friday is the trusted outbound control plane. Claude may propose content through `message.send`, but Friday validates the route and risk, requires exact approval, then writes an immutable `OutboundDelivery`. The existing claim, `DeliveryAttempt`, and dispatcher path is the only component that can invoke a transport.

Scheduled delivery follows the same delivery plane. A `ScheduleDeliveryPolicy` expresses operator intent for future fires. When a due schedule fires, Friday freezes a `ScheduleFireDeliveryPlan` with that fire and execution lineage, route id/fingerprint, route-specific body limit, and `final_agent_summary_v1` content source. The answer materializer reads only the latest effective successful run's canonical `AGENT_FINISHED.summary`, applies Friday's deterministic normalization and content gate, and writes at most one `OutboundDelivery` for the fire. The database's unique `source_schedule_fire_id` constraint is the final exactly-once race fence.

Frozen authority is never rewritten. The dispatcher validates the frozen route against current configuration immediately before the external boundary; missing, disabled, or fingerprint-changed routes result in the safe parked disposition with no transport call or retargeting. Historical plans without a frozen body bound fail closed and are not backfilled from current configuration.

After dispatch begins, uncertainty is persisted as `AMBIGUOUS`; this phase adds no provider idempotency and no automatic redrive of `FAILED` or `AMBIGUOUS` deliveries. It also adds no new transport, scheduler, delivery loop, endpoint type, templates, or Claude-owned delivery authority.
