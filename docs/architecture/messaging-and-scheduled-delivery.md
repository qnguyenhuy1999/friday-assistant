...
The plan freezes the route fingerprint and execution lineage, or a stable suppressed reason when the route is missing or disabled. Policy and route changes therefore affect future fires only; historical fires receive no migration backfill and are never retroactively authorized.
...
The delivery uses only the plan's frozen route id and fingerprint. There is exactly one durable intent per fire, fenced by `OutboundDelivery.source_schedule_fire_id`; materialization does no model or network I/O. Dispatch remains a separate existing component, and it validates route drift against current authority before transport. Historical fires with no Step 6 plan never gain delivery authority retroactively.
