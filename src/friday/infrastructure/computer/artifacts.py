...
from friday.infrastructure.computer.models import Screenshot, SnapshotId
...
class ScreenshotStore:
    ...
    def persist(
        self, screenshot: Screenshot, *, invocation_id: str, snapshot_id: SnapshotId
    ) -> ArtifactCandidate:
        """Validate, write atomically, and return the artifact candidate."""
        self._reject_unusable(screenshot)
        relative = self._relative_location(invocation_id, snapshot_id, screenshot.media_type)
        target = self._settings.workspace_root / relative
    ...
    def _relative_location(
        self, invocation_id: str, snapshot_id: SnapshotId, media_type: str
    ) -> str:
        """`<artifact_root>/<invocation-id>/<snapshot-id>.png`.
        ...
        return f"{root}/{invocation_id}/{snapshot_id.value}{suffix}"
