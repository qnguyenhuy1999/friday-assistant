...
def test_next_sequence_starts_at_one_for_a_new_run(session) -> None:
    store = RunEventStore(session)
    run_id = _make_run(session)
    assert store.next_sequence(run_id) == 1


def test_next_sequence_increments_after_append(session) -> None:
    store = RunEventStore(session)
    run_id = _make_run(session)
    store.append(
        RunEvent(
...
def test_list_for_run_orders_by_sequence(session) -> None:
    store = RunEventStore(session)
    run_id = _make_run(session)
    store.append(
        RunEvent(
...
def test_duplicate_sequence_for_same_run_is_rejected_at_db_level(session) -> None:
    store = RunEventStore(session)
    run_id = _make_run(session)
    store.append(
        RunEvent(
